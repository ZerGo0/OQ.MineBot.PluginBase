        ...
        public async Task<bool> PlaceBlockAt(ILocation location, int itemId, int tickdelay)
        {
            ...
            while (true)
            {
                if (Stopped || !_context.World.GetBlock(location).CanPlaceAt() ||
                    _context.Player.GetLocation().Compare(location))
                {
...
//                await _context.Player.LookAt(location);
        ...
        {
            ZerGo0Debugger.Debug(_context.Player.GetUsername(), $"GoToLocation Destination: {location}");
            var moveResult = await _context.Player.MoveToInteractionRange(location.Offset(-1), mapOptions).Task;
        ...
        public List<ILocation> LayerOpenSpots(ILocation location1, ILocation location2, int currentLayerY)
        {
            var currentLayer = new List<ILocation>();

            var currentLayerStartX = location1.X;
            var currentLayerEndX = location2.X;
            if (location2.X < location1.X)
            {
                currentLayerStartX = location2.X;
                currentLayerEndX = location1.X;
            }

            var currentLayerStartZ = location1.Z;
            var currentLayerEndZ = location2.Z;
            if (location2.Z < location1.Z)
            {
                currentLayerStartZ = location2.Z;
                currentLayerEndZ = location1.Z;
            }


            for (var x = currentLayerStartX; x <= currentLayerEndX; x++)
            for (var z = currentLayerStartZ; z <= currentLayerEndZ; z++)
            {
                var tempLoc = new Location(x, currentLayerY, z);
                if (_context.World.GetBlockId(tempLoc) == 0)
                    currentLayer.Add(tempLoc);
            }
