...
namespace AutoSplashPotion
{
    ...
    public class PluginCore : IStartPlugin
    {
        public override void OnLoad(int version, int subversion, int buildversion)
        {
            ...
            Setting.Add(new NumberSetting("Health Threshold",
                "It will throw the Splash Potion if it's below this value.", 2, 2, 255));
        ...
        public override PluginResponse OnEnable(IBotSettings botSettings)
        {
            if (!Setting.At(2).Get<bool>() && !Setting.At(3).Get<bool>())
                return new PluginResponse(false,
                    "You didn't select any potion type, please check your plugin settings!");
